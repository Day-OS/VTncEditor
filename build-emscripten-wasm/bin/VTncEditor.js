...
var wasmBinaryFile;
  wasmBinaryFile = 'VTncEditor.wasm';
...
var ASM_CONSTS = {
  289660: function() {var file_selector = document.createElement('input'); file_selector.setAttribute('type', 'file'); file_selector.setAttribute('onchange','open_file(event)'); file_selector.setAttribute('accept','.vtnc'); file_selector.click();},  
 289885: function($0) {document.title = UTF8ToString($0);},  
 289922: function($0) {(Module['canvas'].closest('.mn-container') || document.getElementById('container')).className = (['mn-container', AsciiToString($0)]).join(' ');},  
 290071: function() {var element = Module['keyboardListeningElement'] || document; if(element === document) return 1; if(element === window) return 2; if('id' in element) { var bytes = lengthBytesUTF8(element.id) + 1; var memory = _malloc(bytes); stringToUTF8(element.id, memory, bytes); return memory; } return 0;},  
 290369: function($0) {Module['canvas'].style.cursor = AsciiToString($0);},  
 290422: function($0, $1) {var drawEvent = function() { var id = window.requestAnimationFrame(drawEvent); if(!wasmTable.get($0).apply(null, [$1])) { window.cancelAnimationFrame(id); } }; window.requestAnimationFrame(drawEvent);},  
 290627: function() {var id = Module['canvas'].id; var bytes = lengthBytesUTF8(id) + 1; var memory = _malloc(bytes); stringToUTF8(id, memory, bytes); return memory;},  
 290775: function() {var match = navigator.userAgent.match(/Firefox\/(\d+)/); if(match) return match[1]|0; return 0;},  
 290877: function() {var env = ''; if(typeof process !== 'undefined') for(var key in process.env) env += key + '=' + process.env[key] + ''; env += ''; const bytes = lengthBytesUTF8(env) + 1; const memory = _malloc(bytes); stringToUTF8(env, memory, bytes); return memory;},  
 291135: function($0) {var name = UTF8ToString($0); if(typeof process !== 'undefined' && name in process.env) { var env = process.env[name]; var bytes = lengthBytesUTF8(env) + 1; var memory = _malloc(bytes); stringToUTF8(env, memory, bytes); return memory; } return 0;}
};
